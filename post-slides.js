jQuery(document).ready(function($){
      ...
      $('.colorwell')
		...
		// Post the form on theme change
		$('.select-skin select').on('change', function() {
	    $('#post-slides-form').submit();
		});


		$('.min-slide-width input').on('blur', function(){
			var responsiveBase = parseInt($(this).val());
			var responsive = '0 : {items:1}, ';
			for ( var i = 2; i < 7; i++ ) {
				var breakpoint = responsiveBase * i;
				var breakpoint = breakpoint +' : {items:'+ i +'}';
				var responsive = responsive + breakpoint;
				if (i != 6){
					var responsive = responsive + ', ';
				}
			}
			$('.min-slide-width-calc em').text(responsive);
			$('.min-slide-width-calc input').val(responsive);
		});
});
